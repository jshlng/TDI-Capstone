...
rawdf = pd.read_csv(r"C:\Users\joshu\Documents\GitHub\TDI-Capstone\Data Source\BostonListings\listings.csv")
...
rawdf = rawdf[rawdf.zipcode != '33131']

#Remove rows that have unique features (not shared with at least 15 other rows)
rawdf = rawdf[rawdf.property_type != 'Barn']
rawdf = rawdf[rawdf.property_type != 'Boat']
rawdf = rawdf[rawdf.property_type != 'Boutique hotel']
rawdf = rawdf[rawdf.property_type != 'Bungalow']
rawdf = rawdf[rawdf.property_type != 'Casa particular (Cuba)']
rawdf = rawdf[rawdf.property_type != 'Cottage']
rawdf = rawdf[rawdf.property_type != 'Farm stay']
rawdf = rawdf[rawdf.property_type != 'Hostel']
rawdf = rawdf[rawdf.property_type != 'Hotel']
rawdf = rawdf[rawdf.property_type != 'Houseboat']
rawdf = rawdf[rawdf.property_type != 'Villa']
rawdf = rawdf[rawdf.bed_type != 'Couch']
rawdf = rawdf[rawdf.zipcode != '02026']
rawdf = rawdf[rawdf.zipcode != '02139']
rawdf = rawdf[rawdf.zipcode != '02141']
rawdf = rawdf[rawdf.zipcode != '02145']
rawdf = rawdf[rawdf.zipcode != '02149']
rawdf = rawdf[rawdf.zipcode != '02163']
rawdf = rawdf[rawdf.zipcode != '02186']
rawdf = rawdf[rawdf.zipcode != '02445']
rawdf = rawdf[rawdf.zipcode != '02446']
rawdf = rawdf[rawdf.zipcode != '02467']
...
for i, row in rawdf.iterrows():
    ...
    if (isinstance(row['zipcode'], str) and (len(row['zipcode'])>5)):
        rawdf.at[i, 'zipcode'] = row['zipcode'][0:5]
    if (isinstance(row['zipcode'], str) and (len(row['zipcode'])==4)):
        rawdf.at[i, 'zipcode'] = '0' + row['zipcode']

total_prices = np.array(totalpricelist)
...
#List of Amenities selected by hand from the top 50
...
#Encoder for Amenities, limited to 40 of the top Amenities
...
pipe_lin.fit(df, total_prices)
nested_score_lin = cross_val_score(pipe_lin, df, y=total_prices, scoring='neg_mean_absolute_error', cv=outer_cv)

print ("Linear Regression:")
print (nested_score_lin.mean())

label, cat_one_hot_encoder = categorial_transformer.steps[1]
print(cat_one_hot_encoder.categories_)
...
gs_rid.fit(df, total_prices)

nested_score_rid = cross_val_score(gs_rid, df, y=total_prices, scoring='neg_mean_absolute_error', cv=outer_cv)
...
gs_las.fit(df, total_prices)

nested_score_las = cross_val_score(gs_las, df, y=total_prices, scoring='neg_mean_absolute_error', cv=outer_cv)
